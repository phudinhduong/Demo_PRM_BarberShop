...
public class BarberScheduleActivity extends AppCompatActivity {
    ...
    @Override protected void onCreate(Bundle b) {
        ...
        findViewById(R.id.btnLoad).setOnClickListener(v -> load());

        // Hôm nay mặc định (hoặc extra "date" từ BarberHome)
        String d = getIntent().getStringExtra("date");
        if (TextUtils.isEmpty(d)) d = java.time.LocalDate.now().toString();
        etDate.setText(d);

        load(); // tự tải lịch hôm nay
    }
    ...
    private void load() {
        ...
        if (date.isEmpty()) { toast("Nhập ngày (yyyy-MM-dd)"); return; }
    ...
    private void fillShifts(JSONArray arr){
        ...
        for (int i=0;i<arr.length();i++){
            ...
            String line = (s!=null ? (s.optString("start")+"–"+s.optString("end")+" • "+s.optString("status")) : "");
            rowsShifts.add(line);
        }
    }
    ...
    private void fillOffs(JSONArray arr){
        ...
        for (int i=0;i<arr.length();i++){
            ...
            boolean all = o != null && o.optBoolean("allDay");
